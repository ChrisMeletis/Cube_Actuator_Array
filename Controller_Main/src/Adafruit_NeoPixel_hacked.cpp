...
// ~8 KHz slow mode: T0H=40us, T1H=80us, Period=124us
  // Used when ATtiny receiver cannot decode fast NeoPixel signals
  // This runs after both 800/400 KHz paths are skipped via #ifdef,
  // so it only activates when NEO_KHZ400 is not defined.
#if defined(NEO_KHZ8)
  hi = *port |  pinMask;
  lo = *port & ~pinMask;
  interrupts(); // safe to re-enable; timing is loose at 8 KHz
  while (i--) {
    for (uint8_t mask = 0x80; mask; mask >>= 1) {
      if (b & mask) {
        *port = hi;
        delayMicroseconds(80); // T1H
        *port = lo;
        delayMicroseconds(44); // T1L (total period ~124us)
      } else {
        *port = hi;
        delayMicroseconds(40); // T0H
        *port = lo;
        delayMicroseconds(84); // T0L (total period ~124us)
      }
    }
    b = *ptr++;
  }
  noInterrupts();
#endif	//NEO_KHZ8 - custom
